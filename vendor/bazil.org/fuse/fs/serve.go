...
func nodeAttr(ctx context.Context, n Node, attr *fuse.Attr) error { log.DebugLog()
...
//
// Config may be nil.
func New(conn *fuse.Conn, config *Config) *Server { log.DebugLog()
	s := &Server{
		conn:         conn,
...
func (s *Server) Serve(fs FS) error { log.DebugLog()
...
// Server.Serve.
func Serve(c *fuse.Conn, fs FS) error { log.DebugLog()
	server := New(c, nil)
...
func (sn *serveNode) attr(ctx context.Context, attr *fuse.Attr) error { log.DebugLog()
...
func (c *Server) saveNode(inode uint64, node Node) (id fuse.NodeID, gen uint64) { log.DebugLog()
	c.meta.Lock()
	defer c.meta.Unlock()
...
func (c *Server) saveHandle(handle Handle, nodeID fuse.NodeID) (id fuse.HandleID) { log.DebugLog()
	c.meta.Lock()
...
func (n *nodeRefcountDropBug) String() string { log.DebugLog()
	return fmt.Sprintf("bug: trying to drop %d of %d references to %v", n.N, n.Refs, n.Node)
}

func (c *Server) dropNode(id fuse.NodeID, n uint64) (forget bool) { log.DebugLog()
	c.meta.Lock()
	defer c.meta.Unlock()
	snode := c.node[id]
...
func (c *Server) dropHandle(id fuse.HandleID) { log.DebugLog()
	c.meta.Lock()
	c.handle[id] = nil
...
func (m missingHandle) String() string { log.DebugLog()
	return fmt.Sprint("missing handle: ", m.Handle, m.MaxHandle)
}

// Returns nil for invalid handles.
func (c *Server) getHandle(id fuse.HandleID) (shandle *serveHandle) { log.DebugLog()
	c.meta.Lock()
	defer c.meta.Unlock()
...
func (r request) String() string { log.DebugLog()
...
func (m logResponseHeader) String() string { log.DebugLog()
...
func (r response) errstr() string { log.DebugLog()
	s := r.Errno
	if r.Error != "" {
...
func (r response) String() string { log.DebugLog()
	switch {
...
func (n notification) String() string { log.DebugLog()
...
func opName(req fuse.Request) string { log.DebugLog()
...
func (m *logLinkRequestOldNodeNotFound) String() string { log.DebugLog()
...
func (m *renameNewDirNodeNotFound) String() string { log.DebugLog()
...
func (h handlerPanickedError) Error() string { log.DebugLog()
	return fmt.Sprintf("handler panicked: %v", h.Err)
}

var _ fuse.ErrorNumber = handlerPanickedError{}

func (h handlerPanickedError) Errno() fuse.Errno { log.DebugLog()
...
func (h handlerTerminatedError) Error() string { log.DebugLog()
	return fmt.Sprintf("handler terminated (called runtime.Goexit)")
}

var _ fuse.ErrorNumber = handlerTerminatedError{}

func (h handlerTerminatedError) Errno() fuse.Errno { log.DebugLog()
	return fuse.DefaultErrno
}
...
func (e handleNotReaderError) Error() string { log.DebugLog()
	return fmt.Sprintf("handle has no Read: %T", e.handle)
}

var _ fuse.ErrorNumber = handleNotReaderError{}

func (e handleNotReaderError) Errno() fuse.Errno { log.DebugLog()
	return fuse.ENOTSUP
}

func initLookupResponse(s *fuse.LookupResponse) { log.DebugLog()
	s.EntryValid = entryValidTime
}

func (c *Server) serve(r fuse.Request) { log.DebugLog()
...
func (c *Server) handleRequest(ctx context.Context, node Node, snode *serveNode, r fuse.Request, done func(resp interface{})) error { log.DebugLog()
	switch r := r.(type) {
	default:
...
func (c *Server) saveLookup(ctx context.Context, s *fuse.LookupResponse, snode *serveNode, elem string, n2 Node) error { log.DebugLog()
...
func (i invalidateNodeDetail) String() string { log.DebugLog()
	return fmt.Sprintf("Off:%d Size:%d", i.Off, i.Size)
}

func errstr(err error) string { log.DebugLog()
	if err == nil {
		return ""
	}
	return err.Error()
...
func (s *Server) invalidateNode(node Node, off int64, size int64) error { log.DebugLog()
	s.meta.Lock()
...
// the node.
func (s *Server) InvalidateNodeAttr(node Node) error { log.DebugLog()
...
// Returns fuse.ErrNotCached if the kernel is not currently caching
// the node.
func (s *Server) InvalidateNodeData(node Node) error { log.DebugLog()
...
// Returns fuse.ErrNotCached if the kernel is not currently caching
// the node.
func (s *Server) InvalidateNodeDataRange(node Node, off int64, size int64) error { log.DebugLog()
...
func (i invalidateEntryDetail) String() string { log.DebugLog()
...
// node.
func (s *Server) InvalidateEntry(parent Node, name string) error { log.DebugLog()
	s.meta.Lock()
...
func DataHandle(data []byte) Handle { log.DebugLog()
...
func (d *dataHandle) ReadAll(ctx context.Context) ([]byte, error) { log.DebugLog()
	return d.data, nil
}
...
func GenerateDynamicInode(parent uint64, name string) uint64 { log.DebugLog()
	h := fnv.New64a()
	var buf [8]byte
