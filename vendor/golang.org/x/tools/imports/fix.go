...
func importGroup(importPath string) int { log.DebugLog()
...
func dirPackageInfo(srcDir, filename string) (*packageInfo, error) { log.DebugLog()
...
func fixImports(fset *token.FileSet, f *ast.File, filename string) (added []string, err error) { log.DebugLog()
...
func importPathToNameBasic(importPath, srcDir string) (packageName string) { log.DebugLog()
	return path.Base(importPath)
}

// importPathToNameGoPath finds out the actual package name, as declared in its .go files.
// If there's a problem, it falls back to using importPathToNameBasic.
func importPathToNameGoPath(importPath, srcDir string) (packageName string) { log.DebugLog()
...
func importPathToNameGoPathParse(importPath, srcDir string) (packageName string, err error) { log.DebugLog()
...
func (s byImportPathShortLength) Len() int { log.DebugLog() return len(s) }
func (s byImportPathShortLength) Less(i, j int) bool { log.DebugLog()
	vi, vj := s[i].importPathShort, s[j].importPathShort
	return len(vi) < len(vj) || (len(vi) == len(vj) && vi < vj)

}
func (s byImportPathShortLength) Swap(i, j int) { log.DebugLog() s[i], s[j] = s[j], s[i] }

// gate is a semaphore for limiting concurrency.
type gate chan struct{}

func (g gate) enter() { log.DebugLog() g <- struct{}{} }
func (g gate) leave() { log.DebugLog() <-g }
...
func populateIgnore() { log.DebugLog()
...
func populateIgnoredDirs(path string) { log.DebugLog()
...
func skipDir(fi os.FileInfo) bool { log.DebugLog()
...
func shouldTraverse(dir string, fi os.FileInfo) bool { log.DebugLog()
...
func scanGoPath() { log.DebugLog() scanGoDirs(false) }

func scanGoDirs(goRoot bool) { log.DebugLog()
	if Debug {
		which := "$GOROOT"
		if !goRoot {
			which = "$GOPATH"
...
func vendorlessImportPath(ipath string) string { log.DebugLog()
...
func loadExportsGoPath(expectPackage, dir string) map[string]bool { log.DebugLog()
	if Debug {
...
func findImportGoPath(pkgName string, symbols map[string]bool, filename string) (foundPkg string, rename bool, err error) { log.DebugLog()
	if inTests {
		testMu.RLock()
...
func pkgIsCandidate(filename, pkgIdent string, pkg *pkg) bool { log.DebugLog()
...
func hasHyphenOrUpperASCII(s string) bool { log.DebugLog()
	for i := 0; i < len(s); i++ {
		b := s[i]
		if b == '-' || ('A' <= b && b <= 'Z') {
			return true
...
func lowerASCIIAndRemoveHyphen(s string) (ret string) { log.DebugLog()
...
func canUse(filename, dir string) bool { log.DebugLog()
...
func lastTwoComponents(v string) string { log.DebugLog()
	nslash := 0
	for i := len(v) - 1; i >= 0; i-- {
		if v[i] == '/' || v[i] == '\\' {
			nslash++
...
func (fn visitFn) Visit(node ast.Node) ast.Visitor { log.DebugLog()
	return fn(node)
}

func findImportStdlib(shortPkg string, symbols map[string]bool) (importPath string, rename, ok bool) { log.DebugLog()
	for symbol := range symbols {
		key := shortPkg + "." + symbol
		path := stdlib[key]
		if path == "" {
...
func fileInDir(file, dir string) bool { log.DebugLog()
