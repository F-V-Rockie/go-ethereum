...
func (sa *SockaddrDatalink) sockaddr() (unsafe.Pointer, _Socklen, error) { log.DebugLog()
	if sa.Index == 0 {
		return nil, 0, EINVAL
	}
	sa.raw.Len = sa.Len
...
func GetsockoptByte(fd, level, opt int) (value byte, err error) { log.DebugLog()
	var n byte
	vallen := _Socklen(1)
...
func Kevent(kq int, changes, events []Kevent_t, timeout *Timespec) (n int, err error) { log.DebugLog()
...
func sysctlmib(name string, args ...int) ([]_C_int, error) { log.DebugLog()
...
func Sysctl(name string) (string, error) { log.DebugLog()
	return SysctlArgs(name)
}

func SysctlArgs(name string, args ...int) (string, error) { log.DebugLog()
...
func SysctlUint32(name string) (uint32, error) { log.DebugLog()
	return SysctlUint32Args(name)
}

func SysctlUint32Args(name string, args ...int) (uint32, error) { log.DebugLog()
...
func SysctlUint64(name string, args ...int) (uint64, error) { log.DebugLog()
	mib, err := sysctlmib(name, args...)
	if err != nil {
		return 0, err
	}
...
func SysctlRaw(name string, args ...int) ([]byte, error) { log.DebugLog()
	mib, err := sysctlmib(name, args...)
	if err != nil {
		return nil, err
	}
