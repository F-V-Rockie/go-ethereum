...
func getWHATWG(url string) string { log.DebugLog()
...
func getUCM(url string) string { log.DebugLog()
	res, err := http.Get(url)
	if err != nil {
		log.Fatalf("%q: Get: %v", url, err)
	}
...
func (b byRune) Len() int           { log.DebugLog() return len(b) }
func (b byRune) Less(i, j int) bool { log.DebugLog() return b[i]&0xffffff < b[j]&0xffffff }
func (b byRune) Swap(i, j int)      { log.DebugLog() b[i], b[j] = b[j], b[i] }
