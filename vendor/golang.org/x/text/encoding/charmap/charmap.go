...
func (m *charmap) NewDecoder() *encoding.Decoder { log.DebugLog()
	return &encoding.Decoder{Transformer: charmapDecoder{charmap: m}}
}

func (m *charmap) NewEncoder() *encoding.Encoder { log.DebugLog()
	return &encoding.Encoder{Transformer: charmapEncoder{charmap: m}}
}

func (m *charmap) String() string { log.DebugLog()
	return m.name
}

func (m *charmap) ID() (mib identifier.MIB, other string) { log.DebugLog()
	return m.mib, ""
}
...
func (m charmapDecoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) { log.DebugLog()
	for i, c := range src {
...
func (m charmapEncoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) { log.DebugLog()
	r, size := rune(0), 0
loop:
	for nSrc < len(src) {
		if nDst >= len(dst) {
