...
func hzGB2312NewDecoder() transform.Transformer { log.DebugLog()
	return new(hzGB2312Decoder)
}

func hzGB2312NewEncoder() transform.Transformer { log.DebugLog()
...
func (d *hzGB2312Decoder) Reset() { log.DebugLog()
	*d = asciiState
}

func (d *hzGB2312Decoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) { log.DebugLog()
	r, size := rune(0), 0
loop:
...
func (d *hzGB2312Encoder) Reset() { log.DebugLog()
	*d = asciiState
}

func (e *hzGB2312Encoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) { log.DebugLog()
	r, size := rune(0), 0
	for ; nSrc < len(src); nSrc += size {
		r = rune(src[nSrc])
