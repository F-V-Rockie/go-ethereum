...
func iso2022JPNewDecoder() transform.Transformer { log.DebugLog()
	return new(iso2022JPDecoder)
}

func iso2022JPNewEncoder() transform.Transformer { log.DebugLog()
...
func (d *iso2022JPDecoder) Reset() { log.DebugLog()
	*d = asciiState
}

func (d *iso2022JPDecoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) { log.DebugLog()
	r, size := rune(0), 0
loop:
...
func (e *iso2022JPEncoder) Reset() { log.DebugLog()
	*e = asciiState
}

func (e *iso2022JPEncoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) { log.DebugLog()
	r, size := rune(0), 0
	for ; nSrc < len(src); nSrc += size {
		r = rune(src[nSrc])
