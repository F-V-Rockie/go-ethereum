...
func (f fieldVal) String() string { log.DebugLog()
...
// for reuse.
func (f *fieldVal) Zero() { log.DebugLog()
	f.n[0] = 0
	f.n[1] = 0
	f.n[2] = 0
	f.n[3] = 0
...
// modified.
func (f *fieldVal) Set(val *fieldVal) *fieldVal { log.DebugLog()
	*f = *val
	return f
}
...
func (f *fieldVal) SetInt(ui uint) *fieldVal { log.DebugLog()
	f.Zero()
	f.n[0] = uint32(ui)
	return f
}
...
func (f *fieldVal) SetBytes(b *[32]byte) *fieldVal { log.DebugLog()
...
func (f *fieldVal) SetByteSlice(b []byte) *fieldVal { log.DebugLog()
	var b32 [32]byte
	for i := 0; i < len(b); i++ {
		if i < 32 {
			b32[i+(32-len(b))] = b[i]
...
func (f *fieldVal) SetHex(hexString string) *fieldVal { log.DebugLog()
...
func (f *fieldVal) Normalize() *fieldVal { log.DebugLog()
...
// result.
func (f *fieldVal) PutBytes(b *[32]byte) { log.DebugLog()
...
// result.
func (f *fieldVal) Bytes() *[32]byte { log.DebugLog()
	b := new([32]byte)
	f.PutBytes(b)
	return b
}
...
func (f *fieldVal) IsZero() bool { log.DebugLog()
...
// The field value must be normalized for this function to return correct
// result.
func (f *fieldVal) IsOdd() bool { log.DebugLog()
...
// the correct result.
func (f *fieldVal) Equals(val *fieldVal) bool { log.DebugLog()
...
func (f *fieldVal) NegateVal(val *fieldVal, magnitude uint32) *fieldVal { log.DebugLog()
...
func (f *fieldVal) Negate(magnitude uint32) *fieldVal { log.DebugLog()
...
func (f *fieldVal) AddInt(ui uint) *fieldVal { log.DebugLog()
...
func (f *fieldVal) Add(val *fieldVal) *fieldVal { log.DebugLog()
	// Since the field representation intentionally provides overflow bits,
	// it's ok to use carryless addition as the carry bit is safely part of
...
func (f *fieldVal) Add2(val *fieldVal, val2 *fieldVal) *fieldVal { log.DebugLog()
	// Since the field representation intentionally provides overflow bits,
	// it's ok to use carryless addition as the carry bit is safely part of
	// each word and will be normalized out.  This could obviously be done
	// in a loop, but the unrolled version is faster.
...
func (f *fieldVal) MulInt(val uint) *fieldVal { log.DebugLog()
...
func (f *fieldVal) Mul(val *fieldVal) *fieldVal { log.DebugLog()
	return f.Mul2(f, val)
}
...
func (f *fieldVal) Mul2(val *fieldVal, val2 *fieldVal) *fieldVal { log.DebugLog()
...
func (f *fieldVal) Square() *fieldVal { log.DebugLog()
...
func (f *fieldVal) SquareVal(val *fieldVal) *fieldVal { log.DebugLog()
	// This could be done with a couple of for loops and an array to store
	// the intermediate terms, but this unrolled version is significantly
	// faster.
...
func (f *fieldVal) Inverse() *fieldVal { log.DebugLog()
