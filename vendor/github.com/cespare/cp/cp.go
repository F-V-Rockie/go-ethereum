...
func CopyFile(dst, src string) error { log.DebugLog()
	rf, err := os.Open(src)
	if err != nil {
		return err
	}
...
func CopyAll(dst, src string) error { log.DebugLog()
	return filepath.Walk(src, makeWalkFn(dst, src))
}

func makeWalkFn(dst, src string) filepath.WalkFunc { log.DebugLog()
