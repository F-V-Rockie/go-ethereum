...
func (s *session) pickMemdbLevel(umin, umax []byte, maxLevel int) int { log.DebugLog()
	v := s.version()
	defer v.release()
	return v.pickMemdbLevel(umin, umax, maxLevel)
}

func (s *session) flushMemdb(rec *sessionRecord, mdb *memdb.DB, maxLevel int) (int, error) { log.DebugLog()
	// Create sorted table.
...
func (s *session) pickCompaction() *compaction { log.DebugLog()
	v := s.version()
...
func (s *session) getCompactionRange(sourceLevel int, umin, umax []byte, noLimit bool) *compaction { log.DebugLog()
	v := s.version()
...
func newCompaction(s *session, v *version, sourceLevel int, t0 tFiles) *compaction { log.DebugLog()
	c := &compaction{
		s:             s,
		v:             v,
...
func (c *compaction) save() { log.DebugLog()
	c.snapGPI = c.gpi
...
func (c *compaction) restore() { log.DebugLog()
	c.gpi = c.snapGPI
...
func (c *compaction) release() { log.DebugLog()
	if !c.released {
		c.released = true
		c.v.release()
	}
...
func (c *compaction) expand() { log.DebugLog()
...
func (c *compaction) trivial() bool { log.DebugLog()
	return len(c.levels[0]) == 1 && len(c.levels[1]) == 0 && c.gp.size() <= c.maxGPOverlaps
}

func (c *compaction) baseLevelForKey(ukey []byte) bool { log.DebugLog()
...
func (c *compaction) shouldStopBefore(ikey internalKey) bool { log.DebugLog()
...
// Creates an iterator.
func (c *compaction) newIterator() iterator.Iterator { log.DebugLog()
