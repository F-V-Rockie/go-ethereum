...
func (lock *fileStorageLock) Unlock() { log.DebugLog()
	if lock.fs != nil {
		lock.fs.mu.Lock()
...
func OpenFile(path string, readOnly bool) (Storage, error) { log.DebugLog()
...
func (fs *fileStorage) Lock() (Locker, error) { log.DebugLog()
	fs.mu.Lock()
	defer fs.mu.Unlock()
	if fs.open < 0 {
...
func itoa(buf []byte, i int, wid int) []byte { log.DebugLog()
	u := uint(i)
	if u == 0 && wid <= 1 {
		return append(buf, '0')
	}
...
func (fs *fileStorage) printDay(t time.Time) { log.DebugLog()
	if fs.day == t.Day() {
		return
	}
	fs.day = t.Day()
...
func (fs *fileStorage) doLog(t time.Time, str string) { log.DebugLog()
...
func (fs *fileStorage) Log(str string) { log.DebugLog()
	if !fs.readOnly {
		t := time.Now()
		fs.mu.Lock()
		defer fs.mu.Unlock()
...
func (fs *fileStorage) SetMeta(fd FileDesc) (err error) { log.DebugLog()
	if !FileDescOk(fd) {
...
func (fs *fileStorage) GetMeta() (fd FileDesc, err error) { log.DebugLog()
	fs.mu.Lock()
	defer fs.mu.Unlock()
	if fs.open < 0 {
...
func (fs *fileStorage) List(ft FileType) (fds []FileDesc, err error) { log.DebugLog()
	fs.mu.Lock()
	defer fs.mu.Unlock()
	if fs.open < 0 {
		return nil, ErrClosed
...
func (fs *fileStorage) Open(fd FileDesc) (Reader, error) { log.DebugLog()
	if !FileDescOk(fd) {
...
func (fs *fileStorage) Create(fd FileDesc) (Writer, error) { log.DebugLog()
	if !FileDescOk(fd) {
		return nil, ErrInvalidFile
	}
	if fs.readOnly {
...
func (fs *fileStorage) Remove(fd FileDesc) error { log.DebugLog()
	if !FileDescOk(fd) {
		return ErrInvalidFile
	}
	if fs.readOnly {
...
func (fs *fileStorage) Rename(oldfd, newfd FileDesc) error { log.DebugLog()
...
func (fs *fileStorage) Close() error { log.DebugLog()
	fs.mu.Lock()
	defer fs.mu.Unlock()
	if fs.open < 0 {
		return ErrClosed
...
func (fw *fileWrap) Sync() error { log.DebugLog()
...
func (fw *fileWrap) Close() error { log.DebugLog()
	fw.fs.mu.Lock()
...
func fsGenName(fd FileDesc) string { log.DebugLog()
	switch fd.Type {
	case TypeManifest:
...
func fsHasOldName(fd FileDesc) bool { log.DebugLog()
	return fd.Type == TypeTable
}

func fsGenOldName(fd FileDesc) string { log.DebugLog()
	switch fd.Type {
	case TypeTable:
...
func fsParseName(name string) (fd FileDesc, ok bool) { log.DebugLog()
	var tail string
...
func fsParseNamePtr(name string, fd *FileDesc) bool { log.DebugLog()
