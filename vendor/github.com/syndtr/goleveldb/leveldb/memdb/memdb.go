...
func (i *dbIter) fill(checkStart, checkLimit bool) bool { log.DebugLog()
	if i.node != 0 {
...
func (i *dbIter) Error() error { log.DebugLog() return i.err }
...
func (p *DB) randHeight() (h int) { log.DebugLog()
	const branching = 4
	h = 1
...
func (p *DB) findGE(key []byte, prev bool) (int, bool) { log.DebugLog()
	node := 0
	h := p.maxHeight - 1
	for {
...
func (p *DB) findLT(key []byte) int { log.DebugLog()
	node := 0
	h := p.maxHeight - 1
	for {
		next := p.nodeData[node+nNext+h]
...
func (p *DB) findLast() int { log.DebugLog()
	node := 0
	h := p.maxHeight - 1
	for {
		next := p.nodeData[node+nNext+h]
...
func (p *DB) Put(key []byte, value []byte) error { log.DebugLog()
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *DB) Delete(key []byte) error { log.DebugLog()
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *DB) Contains(key []byte) bool { log.DebugLog()
	p.mu.RLock()
...
func (p *DB) Get(key []byte) (value []byte, err error) { log.DebugLog()
	p.mu.RLock()
...
func (p *DB) Find(key []byte) (rkey, value []byte, err error) { log.DebugLog()
	p.mu.RLock()
...
func (p *DB) NewIterator(slice *util.Range) iterator.Iterator { log.DebugLog()
	return &dbIter{p: p, slice: slice}
}

// Capacity returns keys/values buffer capacity.
func (p *DB) Capacity() int { log.DebugLog()
	p.mu.RLock()
	defer p.mu.RUnlock()
	return cap(p.kvData)
}
...
func (p *DB) Size() int { log.DebugLog()
	p.mu.RLock()
	defer p.mu.RUnlock()
	return p.kvSize
}
...
func (p *DB) Free() int { log.DebugLog()
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *DB) Len() int { log.DebugLog()
	p.mu.RLock()
	defer p.mu.RUnlock()
	return p.n
}
...
func New(cmp comparer.BasicComparer, capacity int) *DB { log.DebugLog()
	p := &DB{
		cmp:       cmp,
