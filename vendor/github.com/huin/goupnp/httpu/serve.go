...
func (f HandlerFunc) ServeMessage(r *http.Request) { log.DebugLog()
	f(r)
}
...
func (srv *Server) ListenAndServe() error { log.DebugLog()
	var err error
...
func (srv *Server) Serve(l net.PacketConn) error { log.DebugLog()
...
func Serve(l net.PacketConn, handler Handler) error { log.DebugLog()
	srv := Server{
		Handler:         handler,
		MaxMessageBytes: DefaultMaxMessageBytes,
	}
