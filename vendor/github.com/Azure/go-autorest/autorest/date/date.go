...
func ParseDate(date string) (d Date, err error) { log.DebugLog()
	return parseDate(date, fullDate)
}

func parseDate(date string, format string) (Date, error) { log.DebugLog()
...
// 2006-01-02).
func (d *Date) UnmarshalJSON(data []byte) (err error) { log.DebugLog()
...
// 2006-01-02).
func (d Date) MarshalText() (text []byte, err error) { log.DebugLog()
...
// 2006-01-02).
func (d *Date) UnmarshalText(data []byte) (err error) { log.DebugLog()
	d.Time, err = time.Parse(fullDate, string(data))
	return err
}

// String returns the Date formatted as an RFC3339 full-date string (i.e., 2006-01-02).
func (d Date) String() string { log.DebugLog()
	return fmt.Sprintf(dateFormat, d.Year(), d.Month(), d.Day())
}

// ToTime returns a Date as a time.Time
func (d Date) ToTime() time.Time { log.DebugLog()
	return d.Time
}
