...
// real package name.
func (f Function) String() string { log.DebugLog()
	s, _ := url.QueryUnescape(f.Raw)
	return s
}

// Name is the naked function name.
func (f Function) Name() string { log.DebugLog()
...
func (f Function) PkgName() string { log.DebugLog()
	parts := strings.SplitN(filepath.Base(f.Raw), ".", 2)
	if len(parts) == 1 {
		return ""
	}
...
func (f Function) PkgDotName() string { log.DebugLog()
	parts := strings.SplitN(filepath.Base(f.Raw), ".", 2)
	s, _ := url.QueryUnescape(parts[0])
	if len(parts) == 1 {
		return parts[0]
...
func (f Function) IsExported() bool { log.DebugLog()
	name := f.Name()
...
func (a Args) String() string { log.DebugLog()
	var v []string
...
func (a *Args) Equal(r *Args) bool { log.DebugLog()
...
// equal.
func (a *Args) Similar(r *Args, similar Similarity) bool { log.DebugLog()
	if a.Elided != r.Elided || len(a.Values) != len(r.Values) {
		return false
	}
	if similar == AnyValue {
...
func (a *Args) Merge(r *Args) Args { log.DebugLog()
	out := Args{
...
func (c *Call) Equal(r *Call) bool { log.DebugLog()
...
// equal.
func (c *Call) Similar(r *Call, similar Similarity) bool { log.DebugLog()
...
func (c *Call) Merge(r *Call) Call { log.DebugLog()
	return Call{
...
func (c *Call) IsStdlib() bool { log.DebugLog()
...
func (c *Call) IsPkgMain() bool { log.DebugLog()
...
func (s *Stack) Equal(r *Stack) bool { log.DebugLog()
...
// equal.
func (s *Stack) Similar(r *Stack, similar Similarity) bool { log.DebugLog()
	if len(s.Calls) != len(r.Calls) || s.Elided != r.Elided {
		return false
	}
	for i := range s.Calls {
...
func (s *Stack) Merge(r *Stack) *Stack { log.DebugLog()
...
func (s *Stack) Less(r *Stack) bool { log.DebugLog()
	lStdlib := 0
	lPrivate := 0
...
func (s *Signature) Equal(r *Signature) bool { log.DebugLog()
...
// equal.
func (s *Signature) Similar(r *Signature, similar Similarity) bool { log.DebugLog()
...
func (s *Signature) Merge(r *Signature) *Signature { log.DebugLog()
	min := s.SleepMin
	if r.SleepMin < min {
		min = r.SleepMin
	}
...
// functions is 'more' so it is at the bottom.
func (s *Signature) Less(r *Signature) bool { log.DebugLog()
...
func Bucketize(goroutines []Goroutine, similar Similarity) map[*Signature][]Goroutine { log.DebugLog()
...
func (b *Bucket) First() bool { log.DebugLog()
...
func (b *Bucket) Less(r *Bucket) bool { log.DebugLog()
	if b.First() {
		return true
	}
	if r.First() {
...
func (b Buckets) Len() int { log.DebugLog()
	return len(b)
}

func (b Buckets) Less(i, j int) bool { log.DebugLog()
	return b[i].Less(&b[j])
}

func (b Buckets) Swap(i, j int) { log.DebugLog()
	b[j], b[i] = b[i], b[j]
}

// SortBuckets creates a list of Bucket from each goroutine stack trace count.
func SortBuckets(buckets map[*Signature][]Goroutine) Buckets { log.DebugLog()
...
func scanLines(data []byte, atEOF bool) (advance int, token []byte, err error) { log.DebugLog()
	if atEOF && len(data) == 0 {
		return 0, nil, nil
	}
...
func ParseDump(r io.Reader, out io.Writer) ([]Goroutine, error) { log.DebugLog()
...
func nameArguments(goroutines []Goroutine) { log.DebugLog()
...
func (a uint64Slice) Len() int           { log.DebugLog() return len(a) }
func (a uint64Slice) Swap(i, j int)      { log.DebugLog() a[i], a[j] = a[j], a[i] }
func (a uint64Slice) Less(i, j int) bool { log.DebugLog() return a[i] < a[j] }
