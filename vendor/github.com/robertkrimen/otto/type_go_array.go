...
func (runtime *_runtime) newGoArrayObject(value reflect.Value) *_object { log.DebugLog()
...
func _newGoArrayObject(value reflect.Value) *_goArrayObject { log.DebugLog()
...
func (self _goArrayObject) getValue(index int64) (reflect.Value, bool) { log.DebugLog()
...
func (self _goArrayObject) setValue(index int64, value Value) bool { log.DebugLog()
...
func goArrayGetOwnProperty(self *_object, name string) *_property { log.DebugLog()
	// length
	if name == "length" {
		return &_property{
...
func goArrayEnumerate(self *_object, all bool, each func(string) bool) { log.DebugLog()
	object := self.value.(*_goArrayObject)
	// .0, .1, .2, ...
...
func goArrayDefineOwnProperty(self *_object, name string, descriptor _property, throw bool) bool { log.DebugLog()
	if name == "length" {
...
func goArrayDelete(self *_object, name string, throw bool) bool { log.DebugLog()
	// length
	if name == "length" {
		return self.runtime.typeErrorResult(throw)
	}
