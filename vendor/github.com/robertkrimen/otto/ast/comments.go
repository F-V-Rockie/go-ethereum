...
func NewComment(text string, idx file.Idx) *Comment { log.DebugLog()
	comment := &Comment{
		Begin:    idx,
		Text:     text,
		Position: TBD,
...
func (cp CommentPosition) String() string { log.DebugLog()
	switch cp {
	case LEADING:
		return "Leading"
	case TRAILING:
...
func (c Comment) String() string { log.DebugLog()
...
func NewComments() *Comments { log.DebugLog()
	comments := &Comments{
		CommentMap: CommentMap{},
	}
...
func (c *Comments) String() string { log.DebugLog()
...
func (c *Comments) FetchAll() []*Comment { log.DebugLog()
	defer func() {
		c.Comments = nil
		c.future = nil
	}()
...
func (c *Comments) Fetch() []*Comment { log.DebugLog()
	defer func() {
		c.Comments = nil
	}()
...
func (c *Comments) MarkComments(position CommentPosition) { log.DebugLog()
...
func (c *Comments) Unset() { log.DebugLog()
	if c.Current != nil {
...
func (c *Comments) SetExpression(node Expression) { log.DebugLog()
	// Skipping same node
	if c.Current == node {
		return
	}
...
func (c *Comments) PostProcessNode(node Node) { log.DebugLog()
...
func (c *Comments) applyComments(node, previous Node, position CommentPosition) { log.DebugLog()
	if previous != nil {
...
func (c *Comments) AtLineBreak() { log.DebugLog()
	c.wasLineBreak = true
}
...
func (cm CommentMap) AddComment(node Node, comment *Comment) { log.DebugLog()
	list := cm[node]
...
func (cm CommentMap) AddComments(node Node, comments []*Comment, position CommentPosition) { log.DebugLog()
...
func (cm CommentMap) Size() int { log.DebugLog()
	size := 0
...
func (cm CommentMap) MoveComments(from, to Node, position CommentPosition) { log.DebugLog()
