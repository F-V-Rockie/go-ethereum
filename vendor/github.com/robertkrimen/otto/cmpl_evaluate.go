...
func (self *_runtime) cmpl_evaluate_nodeProgram(node *_nodeProgram, eval bool) Value { log.DebugLog()
	if !eval {
...
func (self *_runtime) cmpl_call_nodeFunction(function *_object, stash *_fnStash, node *_nodeFunctionLiteral, this Value, argumentList []Value) Value { log.DebugLog()
...
func (self *_runtime) cmpl_functionDeclaration(list []*_nodeFunctionLiteral) { log.DebugLog()
...
func (self *_runtime) cmpl_variableDeclaration(list []string) { log.DebugLog()
	executionContext := self.scope
	eval := executionContext.eval
	stash := executionContext.variable
