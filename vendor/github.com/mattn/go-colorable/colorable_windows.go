...
func NewColorable(file *os.File) io.Writer { log.DebugLog()
	if file == nil {
...
func NewColorableStdout() io.Writer { log.DebugLog()
	return NewColorable(os.Stdout)
}

// NewColorableStderr return new instance of Writer which handle escape sequence for stderr.
func NewColorableStderr() io.Writer { log.DebugLog()
...
func (c consoleColor) foregroundAttr() (attr word) { log.DebugLog()
	if c.red {
		attr |= foregroundRed
	}
	if c.green {
...
func (c consoleColor) backgroundAttr() (attr word) { log.DebugLog()
	if c.red {
		attr |= backgroundRed
	}
	if c.green {
...
func (a hsv) dist(b hsv) float32 { log.DebugLog()
	dh := a.h - b.h
	switch {
	case dh > 0.5:
		dh = 1 - dh
...
func toHSV(rgb int) hsv { log.DebugLog()
...
func toHSVTable(rgbTable []consoleColor) hsvTable { log.DebugLog()
...
func (t hsvTable) find(rgb int) consoleColor { log.DebugLog()
	hsv := toHSV(rgb)
	n := 7
	l := float32(5.0)
	for i, p := range t {
...
func minmax3f(a, b, c float32) (min, max float32) { log.DebugLog()
	if a < b {
		if b < c {
			return a, c
		} else if a < c {
