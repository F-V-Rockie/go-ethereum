...
func Bytes(in []byte) []byte { log.DebugLog()
...
func IsEscaped(b []byte) bool { log.DebugLog()
	for len(b) > 0 {
		i := bytes.IndexByte(b, '\\')
		if i < 0 {
			return false
...
func AppendUnescaped(dst, src []byte) []byte { log.DebugLog()
	var pos int
	for len(src) > 0 {
...
func Unescape(in []byte) []byte { log.DebugLog()
	if len(in) == 0 {
		return nil
	}
