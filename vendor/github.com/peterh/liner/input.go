...
func (s *State) startPrompt() { log.DebugLog()
...
func (s *State) inputWaiting() bool { log.DebugLog()
	return len(s.next) > 0
}

func (s *State) restartPrompt() { log.DebugLog()
...
func (s *State) stopPrompt() { log.DebugLog()
	if s.terminalSupported {
		s.defaultMode.ApplyMode()
	}
}

func (s *State) nextPending(timeout <-chan time.Time) (rune, error) { log.DebugLog()
	select {
...
func (s *State) readNext() (interface{}, error) { log.DebugLog()
	if len(s.pending) > 0 {
		rv := s.pending[0]
		s.pending = s.pending[1:]
		return rv, nil
