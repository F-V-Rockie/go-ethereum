...
func countGlyphs(s []rune) int { log.DebugLog()
	n := 0
	for _, r := range s {
...
func countMultiLineGlyphs(s []rune, columns int, start int) int { log.DebugLog()
	n := start
	for _, r := range s {
		if r < 127 {
			n++
...
func getPrefixGlyphs(s []rune, num int) []rune { log.DebugLog()
	p := 0
...
func getSuffixGlyphs(s []rune, num int) []rune { log.DebugLog()
	p := len(s)
	for n := 0; n < num && p > 0; p-- {
		// speed up the common case
		if s[p-1] < 127 {
