...
func errnotexist(name string) error { log.DebugLog()
	return &os.PathError{
		Op:   "Node",
		Path: name,
		Err:  os.ErrNotExist,
...
func newnode(name string) node { log.DebugLog()
	return node{
		Name:  name,
...
func (nd node) addchild(name, base string) node { log.DebugLog()
...
func (nd node) Add(name string) node { log.DebugLog()
...
func (nd node) AddDir(fn walkFunc) error { log.DebugLog()
	stack := []node{nd}
Traverse:
...
func (nd node) Get(name string) (node, error) { log.DebugLog()
	i := indexbase(nd.Name, name)
	if i == -1 {
...
func (nd node) Del(name string) error { log.DebugLog()
	i := indexbase(nd.Name, name)
	if i == -1 {
...
func (nd node) Walk(fn walkFunc) error { log.DebugLog()
	stack := []node{nd}
Traverse:
	for n := len(stack); n != 0; n = len(stack) {
		nd, stack = stack[n-1], stack[:n-1]
...
func (nd node) WalkPath(name string, fn walkPathFunc) error { log.DebugLog()
	i := indexbase(nd.Name, name)
	if i == -1 {
		return errnotexist(name)
	}
...
func (r root) addroot(name string) node { log.DebugLog()
...
func (r root) root(name string) (node, error) { log.DebugLog()
	if vol := filepath.VolumeName(name); vol != "" {
...
func (r root) Add(name string) node { log.DebugLog()
	return r.addroot(name).Add(name)
}

func (r root) AddDir(dir string, fn walkFunc) error { log.DebugLog()
	return r.Add(dir).AddDir(fn)
}

func (r root) Del(name string) error { log.DebugLog()
	nd, err := r.root(name)
	if err != nil {
		return err
	}
...
func (r root) Get(name string) (node, error) { log.DebugLog()
	nd, err := r.root(name)
	if err != nil {
		return node{}, err
	}
...
func (r root) Walk(name string, fn walkFunc) error { log.DebugLog()
	nd, err := r.Get(name)
	if err != nil {
		return err
	}
...
func (r root) WalkPath(name string, fn walkPathFunc) error { log.DebugLog()
	nd, err := r.root(name)
	if err != nil {
		return err
	}
