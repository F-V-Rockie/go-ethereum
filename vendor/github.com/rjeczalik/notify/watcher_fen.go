...
func newTrigger(pthLkp map[string]*watched) trigger { log.DebugLog()
	return &fen{
		pthLkp: pthLkp,
		cf:     newCfen(),
	}
...
func (f *fen) Stop() error { log.DebugLog()
	return f.cf.portAlert(f.p)
}

// Close implements trigger.
func (f *fen) Close() (err error) { log.DebugLog()
	return syscall.Close(f.p)
}

// NewWatched implements trigger.
func (*fen) NewWatched(p string, fi os.FileInfo) (*watched, error) { log.DebugLog()
	return &watched{trgWatched{p: p, fi: fi}}, nil
}

// Record implements trigger.
func (f *fen) Record(w *watched) { log.DebugLog()
	f.pthLkp[w.p] = w
}

// Del implements trigger.
func (f *fen) Del(w *watched) { log.DebugLog()
	delete(f.pthLkp, w.p)
}

func inter2pe(n interface{}) PortEvent { log.DebugLog()
	pe, ok := n.(PortEvent)
	if !ok {
...
func (f *fen) Watched(n interface{}) (*watched, int64, error) { log.DebugLog()
	pe := inter2pe(n)
...
func (f *fen) Init() (err error) { log.DebugLog()
	f.p, err = f.cf.portCreate()
	return
}

func fi2fo(fi os.FileInfo, p string) FileObj { log.DebugLog()
...
func (f *fen) Unwatch(w *watched) error { log.DebugLog()
	return f.cf.portDissociate(f.p, FileObj{Name: w.p})
}

// Watch implements trigger.
func (f *fen) Watch(fi os.FileInfo, w *watched, e int64) error { log.DebugLog()
	return f.cf.portAssociate(f.p, fi2fo(fi, w.p), int(e))
}

// Wait implements trigger.
func (f *fen) Wait() (interface{}, error) { log.DebugLog()
	var (
		pe  PortEvent
		err error
	)
...
func (f *fen) IsStop(n interface{}, err error) bool { log.DebugLog()
