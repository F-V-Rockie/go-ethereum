...
func (diff eventDiff) Event() Event { log.DebugLog()
...
func (wp watchpoint) dryAdd(ch chan<- EventInfo, e Event) eventDiff { log.DebugLog()
...
func (wp watchpoint) Add(c chan<- EventInfo, e Event) (diff eventDiff) { log.DebugLog()
	wp[c] |= e
	diff[0] = wp[nil]
	diff[1] = diff[0] | e
	wp[nil] = diff[1] &^ omit
...
func (wp watchpoint) Del(c chan<- EventInfo, e Event) (diff eventDiff) { log.DebugLog()
	wp[c] &^= e
	if wp[c] == 0 {
		delete(wp, c)
	}
...
func (wp watchpoint) Dispatch(ei EventInfo, extra Event) { log.DebugLog()
...
func (wp watchpoint) Total() Event { log.DebugLog()
	return wp[nil] &^ internal
}

func (wp watchpoint) IsRecursive() bool { log.DebugLog()
