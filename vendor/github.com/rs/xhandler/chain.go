...
// function signatures.
func (c *Chain) Add(f ...interface{}) { log.DebugLog()
	for _, h := range f {
		switch v := h.(type) {
...
func (c *Chain) With(f ...interface{}) *Chain { log.DebugLog()
	n := make(Chain, len(*c))
	copy(n, *c)
	n.Add(f...)
	return &n
...
func (c *Chain) UseC(f func(next HandlerC) HandlerC) { log.DebugLog()
	*c = append(*c, f)
}
...
func (c *Chain) Use(f func(next http.Handler) http.Handler) { log.DebugLog()
...
func (c Chain) Handler(xh HandlerC) http.Handler { log.DebugLog()
...
func (c Chain) HandlerFC(xhf HandlerFuncC) http.Handler { log.DebugLog()
	ctx := context.Background()
...
func (c Chain) HandlerH(h http.Handler) http.Handler { log.DebugLog()
	ctx := context.Background()
...
func (c Chain) HandlerF(hf http.HandlerFunc) http.Handler { log.DebugLog()
	ctx := context.Background()
	return c.HandlerCtx(ctx, HandlerFuncC(func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		hf(w, r)
	}))
...
// the chain and returns a new standard http.Handler instance.
func (c Chain) HandlerCtx(ctx context.Context, xh HandlerC) http.Handler { log.DebugLog()
...
func (c Chain) HandlerC(xh HandlerC) HandlerC { log.DebugLog()
	for i := len(c) - 1; i >= 0; i-- {
		xh = c[i](xh)
	}
	return xh
...
func (c Chain) HandlerCF(xhc HandlerFuncC) HandlerC { log.DebugLog()
