...
func prettyPrint(vm *otto.Otto, value otto.Value, w io.Writer) { log.DebugLog()
	ppctx{vm: vm, w: w}.printValue(value, 0, false)
}

// prettyError writes err to standard output.
func prettyError(vm *otto.Otto, err error, w io.Writer) { log.DebugLog()
	failure := err.Error()
...
func (re *JSRE) prettyPrintJS(call otto.FunctionCall) otto.Value { log.DebugLog()
...
func (ctx ppctx) indent(level int) string { log.DebugLog()
	return strings.Repeat(indentString, level)
}

func (ctx ppctx) printValue(v otto.Value, level int, inArray bool) { log.DebugLog()
	switch {
	case v.IsObject():
...
func (ctx ppctx) printObject(obj *otto.Object, level int, inArray bool) { log.DebugLog()
	switch obj.Class() {
	case "Array", "GoArray":
		lv, _ := obj.Get("length")
		len, _ := lv.ToInteger()
...
func (ctx ppctx) fields(obj *otto.Object) []string { log.DebugLog()
	var (
		vals, methods []string
...
func iterOwnAndConstructorKeys(vm *otto.Otto, obj *otto.Object, f func(string)) { log.DebugLog()
	seen := make(map[string]bool)
...
func iterOwnKeys(vm *otto.Otto, obj *otto.Object, f func(string)) { log.DebugLog()
...
func (ctx ppctx) isBigNumber(v *otto.Object) bool { log.DebugLog()
...
func toString(obj *otto.Object) string { log.DebugLog()
	s, _ := obj.Call("toString")
	return s.String()
}

func constructorPrototype(obj *otto.Object) *otto.Object { log.DebugLog()
