...
func newTestTransport(id discover.NodeID, fd net.Conn) transport { log.DebugLog()
...
func (c *testTransport) doEncHandshake(prv *ecdsa.PrivateKey, dialDest *discover.Node) (discover.NodeID, error) { log.DebugLog()
	return c.id, nil
}

func (c *testTransport) doProtoHandshake(our *protoHandshake) (*protoHandshake, error) { log.DebugLog()
	return &protoHandshake{ID: c.id, Name: "test"}, nil
}

func (c *testTransport) close(err error) { log.DebugLog()
	c.rlpx.fd.Close()
	c.closeErr = err
}

func startTestServer(t *testing.T, id discover.NodeID, pf func(*Peer)) *Server { log.DebugLog()
	config := Config{
		Name:       "test",
		MaxPeers:   10,
...
func TestServerListen(t *testing.T) { log.DebugLog()
...
func TestServerDial(t *testing.T) { log.DebugLog()
...
func TestServerTaskScheduling(t *testing.T) { log.DebugLog()
	var (
...
func TestServerManyTasks(t *testing.T) { log.DebugLog()
...
func (tg taskgen) newTasks(running int, peers map[discover.NodeID]*Peer, now time.Time) []task { log.DebugLog()
	return tg.newFunc(running, peers)
}
func (tg taskgen) taskDone(t task, now time.Time) { log.DebugLog()
	tg.doneFunc(t)
}
func (tg taskgen) addStatic(*discover.Node) { log.DebugLog()
}
func (tg taskgen) removeStatic(*discover.Node) { log.DebugLog()
}
...
func (t *testTask) Do(srv *Server) { log.DebugLog()
	t.called = true
}
...
func TestServerAtCap(t *testing.T) { log.DebugLog()
	trustedID := randomID()
	srv := &Server{
		Config: Config{
			PrivateKey:   newkey(),
...
func TestServerSetupConn(t *testing.T) { log.DebugLog()
	id := randomID()
	srvkey := newkey()
...
func (c *setupTransport) doEncHandshake(prv *ecdsa.PrivateKey, dialDest *discover.Node) (discover.NodeID, error) { log.DebugLog()
	c.calls += "doEncHandshake,"
	return c.id, c.encHandshakeErr
}
func (c *setupTransport) doProtoHandshake(our *protoHandshake) (*protoHandshake, error) { log.DebugLog()
...
func (c *setupTransport) close(err error) { log.DebugLog()
	c.calls += "close,"
	c.closeErr = err
}

// setupConn shouldn't write to/read from the connection.
func (c *setupTransport) WriteMsg(Msg) error { log.DebugLog()
	panic("WriteMsg called on setupTransport")
}
func (c *setupTransport) ReadMsg() (Msg, error) { log.DebugLog()
...
func randomID() (id discover.NodeID) { log.DebugLog()
	for i := range id {
