...
func NewDefaultHiveParams() *HiveParams { log.DebugLog()
...
//have been evaluated
func (self *HiveParams) Init(path string) { log.DebugLog()
	self.KadDbPath = filepath.Join(path, "bzz-peers.json")
}

func NewHive(addr common.Hash, params *HiveParams, swapEnabled, syncEnabled bool) *Hive { log.DebugLog()
...
func (self *Hive) SyncEnabled(on bool) { log.DebugLog()
	self.syncEnabled = on
}

func (self *Hive) SwapEnabled(on bool) { log.DebugLog()
	self.swapEnabled = on
}

func (self *Hive) BlockNetworkRead(on bool) { log.DebugLog()
	self.blockRead = on
}

func (self *Hive) BlockNetworkWrite(on bool) { log.DebugLog()
	self.blockWrite = on
}

// public accessor to the hive base address
func (self *Hive) Addr() kademlia.Address { log.DebugLog()
	return self.addr
}
...
func (self *Hive) Start(id discover.NodeID, listenAddr func() string, connectPeer func(string) error) (err error) { log.DebugLog()
...
func (self *Hive) keepAlive() { log.DebugLog()
...
func (self *Hive) Stop() error { log.DebugLog()
...
func (self *Hive) addPeer(p *peer) error { log.DebugLog()
...
func (self *Hive) removePeer(p *peer) { log.DebugLog()
...
func (self *Hive) getPeers(target storage.Key, max int) (peers []*peer) { log.DebugLog()
...
func (self *Hive) DropAll() { log.DebugLog()
...
func newNodeRecord(addr *peerAddr) *kademlia.NodeRecord { log.DebugLog()
	now := time.Now()
...
func (self *Hive) HandlePeersMsg(req *peersMsgData, from *peer) { log.DebugLog()
...
func (self *peer) Addr() kademlia.Address { log.DebugLog()
	return self.remoteAddr.Addr
}

func (self *peer) Url() string { log.DebugLog()
	return self.remoteAddr.String()
}

// TODO take into account traffic
func (self *peer) LastActive() time.Time { log.DebugLog()
...
func loadSync(record *kademlia.NodeRecord, node kademlia.Node) error { log.DebugLog()
	p, ok := node.(*peer)
	if !ok {
...
func saveSync(record *kademlia.NodeRecord, node kademlia.Node) { log.DebugLog()
...
func (self *Hive) peers(req *retrieveRequestMsgData) { log.DebugLog()
	if req != nil {
		var addrs []*peerAddr
...
func (self *Hive) String() string { log.DebugLog()
